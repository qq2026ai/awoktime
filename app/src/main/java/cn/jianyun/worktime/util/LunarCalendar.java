...
public class LunarCalendar {
    ...
    final static String monthTag[] = {"正", "二", "三", "四", "五", "六", "七", "八", "九", "十", "冬", "腊"};
    ...
    public static String getLunarDay(Date date) {
        String dstr = MyDateTool.toDateString(date);
        String lunarDay = recentLanarDayMap.get(dstr);
        if(lunarDay != null){
            return lunarDay;
        }
        int[] s = solarToLunar(date.getYear() + 1900, date.getMonth() + 1, date.getDate());
        int day = s[2];
        int month = s[1];
        if(day == 1){
            return monthTag[month -1] + "月";
        }
        return getChinaDayString(s[2]);
    }

    //把最近1年的日期都写入缓存，避免重复计算
    public static Map<String, String> recentLanarDayMap = new HashMap<>();
    ...
    private static void init(){
        Date d = new Date();
        for (int i = -90; i < 90; i++) {
            Date dd = MyDateTool.gapDay(d, i);
            recentLanarDayMap.put(MyDateTool.toDateString(dd), getLunarDay(dd));
        }
    }
